...
| Fixed products | Q15, rounded result, high-word products and carry variants; sine/cosine exhausted over all 65536 angles with integration goldens on three sortie profiles | Call-site audit complete: every `fixedMulQ14`/`sinMul`/`cosMul`/`rotateVectorComponent` caller is render/HUD-boundary (camera-space projection, sphere rings, HUD markers, spark scatter) or typed-routed (`TrackMath`/`CamMath`/`RotationMath`); the sim-globals declaration ratchet plus the locals sweep leave no unreviewed narrowing stores |
| Long arithmetic | WordPair32, shifts, full/saturated division | Per-op edge policy lives in `fixed_math.hpp` comments at each helper: `WordPair32::join` widens via int64 (no UB), `shiftLongLeft` is an unsigned 32-bit wrap, `arithmeticShiftRight` is MSC-style floor division including INT32_MIN, `abs16Compatible` preserves the -32768 self-map, `clampRange` wraps ≤-0x4000 to max, `signedRatio16Bits` has no div-by-zero guard by contract (callers provide nonzero denominators — the original faults the same way), `highWordWrapped` reproduces the DX-overflow read. `signedRatio16`/`computeBearing`/`valueToAngle`/`complementAngle` stay as test-pinned oracles |
| Orientation | Typed persistent matrix/Euler state, fixed/double recovery, axis deltas, render pose interpolation; control inputs (`g_pitchInput`/`g_rollInput` typed commands); modern refresh integrated via `recover().needsRefresh` porting the original pole-band + roll→0 triggers | Modern pole-fold canonicalization verified by the loop profile on both backends |
| Projection/HUD | Selected projection, clipping, HUD rotation helpers; the `-1` invalid/depth sentinel is pinned by `geometry_behavior_tests` (behind-camera rejection) and exercised through the sortie camera-hash goldens | Residual: render-internal projection math in egtacmap/eg3drast stays at the word boundary by design |
...
| START/mission-gen | stmath's `approxDistance`/`calcBearing`/`clampValue` were verbatim copies of the egame DOS kernels; they now delegate to `f15::fixed::rangeApprox`/`computeBearing`/`clampRange`, with `stmath_kernel_tests` pinning full-domain equivalence first. `MAP_Y_MIRROR` names the world-file↔map Y-axis flip shared by stgen, egtgt2, egcombat, egframe, egflight and egtarget. stgen's generation constants (mission range scale, cell snap, spawn offsets/altitudes, fuel scale, small-base fine-Y correction) are named. The generator still runs once at setup producing packed words — no fractional path can survive into the word tables by contract. `FlightUnit.xPrecise`/`yPrecise` are typed-routed through `objectFineSet` in `worldxfer.c`; `baseXPrecise`/`baseYPrecise` are write-only in this port | `mission_gen_tests` pins `runGenerator` end-to-end across missionPick 0/1/-1 on a seeded synthetic world AND on the real SVN theater: `SVN.WLD.json` (objects/bases/terrain grid/type table/names) + `VN.3DG/.3DT.json` (real sparse quadtree — six populated L1 cells, L2 tiles invisible to the type gate). Real-data runs snap targets onto real editor-aligned objects, retry through real impassable terrain, and pin the pick-0 degenerate path when fixed table coords miss all populated cells. That degenerate path exposed a genuine port defect: `targetIdx == -1` flowed into `worldObjects[-1]` reads AND an OOB `unitRef` write (harmless adjacent memory on DOS, undefined here). `runGenerator` now clamps every object-table read through `objectSlotOrZero` while the Target records keep the `-1` sentinel, and `getItemCoordStr`/`buildTargetLabel` clamp internally so briefing-side callers stay defined too. Remaining gap: binary asset parse itself (covered separately by `start_parse_behavior_tests`) |
