...
int main(int argc, char *argv[]) {
    ...
    for (size_t i = 0; i < 0x18; ++i) {
        if (ovh.description[i] != 0) descLen++;
    }
    ...
    smatch match;
    if (regex_match(patchArg, match, IDC_RE))
        generateIdc(match, ovh, slots);
    else if (regex_match(patchArg, match, PATCHEXE_RE))
        patchExe(match, ovh, slots);
    else {
