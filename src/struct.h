...
struct SimObject {
    ...
    union {
        int16 w;
        uint8 b[2];
    } heading;   // +0x10  yaw; b[1] high byte = 180deg flip
    int16 pitch; // +0x12
    union {
        int16 w;
        uint8 b[2];
    } bank;     // +0x14  roll/bank (3rd Euler arg); b[1] high-byte flip
    int16 spec; // +0x16  index into aircraftTypes
    union {
        uint16 w;
        uint8 b[2];
    } flags;            // +0x18  b[0] low byte, b[1] high byte
...
/* PageDesc: page descriptor used by the graphics layer.
 * Callers pass a pointer to this struct (as int16 *) and the gfx layer reads
 * fields by word index — word 0 pageNum, word 3 color, word 6 font.
 * C code accesses color/font directly. */
#pragma pack(1)
struct PageDesc {
...
#pragma pack()
STATIC_ASSERT(sizeof(struct PageDesc) == 22);
...
struct VertexProj {
    ...
    union {
        int32 v[121];
        int16 lo;
    } x; /* projected screen X; .lo = HUD X */
    union {
        int32 v[121];
        int16 lo;
    } y;                 /* projected screen Y; .lo = HUD Y */
