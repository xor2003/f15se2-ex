...
void fixedMath() {
    ...
    for (int value = 0; value < 65536; ++value) {
        for (int speed : {-65536, -32768, -1, 0, 1, 2700, 8100, 32767, 65536, INT32_MAX - 1, INT32_MIN + 2})
            require(legacy::signedAngle(FM::liftCorrection(FC::speed(word(value)), FC::speed(speed))) == lift(value, speed),
                    "lift signed quotient/word clamp changed");
        for (int stall : {-32768, -1, 0, 1, 2700, 32767})
            require(legacy::signedAngle(FM::liftCorrection(FC::speed(stall), FC::speed(value))) == lift(stall, value),
    ...
    for (int bad : {INT32_MIN, INT32_MIN + 1, INT32_MAX})
        rejects([&] { FM::liftCorrection(FC::speed(2700), FC::speed(bad)); });
...
void modernMath() {
    ...
    for (int tick = 0; tick < 12000; ++tick) {
        ...
        const auto correction = MM::liftCorrection(MC::speed(2700.375), MC::speed(speed));
    ...
    require(Boundary<M>::radians(MM::liftCorrection(MC::speed(32767.5), {})) == 8192 * radiansPerUnit,
            "modern lift clamp wraps");
    require(Boundary<M>::radians(MM::liftCorrection(MC::speed(-0.25), {})) == 0,
    ...
    require(std::isfinite(Boundary<M>::radians(MM::liftCorrection(MC::speed(1e308), MC::speed(1e308)))),
            "modern lift ratio overflowed before division");
    rejects([] { MM::liftCorrection(MC::speed(1e308), {}); });
}
}
